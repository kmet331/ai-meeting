...
주제 안의 [현재 질문에 대한 임시 해석]을 실행 기준으로 삼는다. 아직 '사용자가 실제로 받고 싶은 반응'에 들어갈 구체적인 내용이 없고 가까운 다른 과업만 논의했다면 대화가 요청에 답한 것으로 보지 않는다. 원문이 특정 범위의 서로 다른 실제 답을 명시적으로 요구할 때 그 남은 답은 발언 균등이 아니라 요청 자체의 남은 내용이다.
이미 서로 다른 구체적 관점·예외·대가가 나왔지만 그 관계에 아무도 반응하지 않았다면, 그 관계를 실제로 짚을 사람이 있을 때 CONTINUE다. 의견을 나열한 뒤 곧바로 끝내지 않는다. 단, 같은 뜻을 말만 바꾼 경우나 새 연결이 없는 경우에는 시간을 채우기 위해 이어가지 않는다.
...
주제 안의 요청 프레임을 참고 장식으로 두지 않는다. 선택된 사람의 target과 impulse는 '사용자가 실제로 받고 싶은 반응'에 직접 기여해야 하며, 대신 하면 안 되는 가까운 다른 과업으로 새지 않아야 한다.
...
주제 안의 요청 프레임은 내가 무엇을 말해야 하는지 정하는 실행 기준이다. 내 발언이 '사용자가 실제로 받고 싶은 반응' 자체를 내놓는지 확인한다. 사용자가 방법을 묻지 않았다면 순서·형식·시간·준비 절차를 제안하는 것으로 실제 답을 대신하지 않는다. 여러 기대가 함께 있으면 지금 자연스럽게 기여할 부분을 말하되 다른 기대를 지워버리지 않는다.
